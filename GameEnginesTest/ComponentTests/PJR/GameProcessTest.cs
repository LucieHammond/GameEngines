...
namespace GameEnginesTest.ComponentTests.PJR
{
    ...
    public class GameProcessTest
    {
        ...
        public GameProcessTest()
        {
            ...
            // Create an empty process with no services and no rules 
            // This is to avoid errors due to interactions with rules that will be tested in integration tests
        ...
        public void PerformCompleteLifeCycle()
        {
            ...
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.ServiceHandler.IsOperational));
            Assert.IsNotNull(m_Process.ServiceProvider);
            Assert.IsNotNull(m_Process.CurrentGameMode);
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.CurrentGameMode.IsOperational));
            m_Process.Update();

            // Srop process : Unload game mode and services
            m_Process.Stop();
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.CurrentGameMode == null));
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.ServiceHandler == null));
            Assert.IsTrue(!m_Process.IsRunning);
        }
        ...
        public void CanSwitchGameMode()
        {
            ...
            m_Process.Start();
            SimulateProcessUntil(() => m_Process.ServiceHandler.IsOperational);
            SimulateProcessUntil(() => m_Process.CurrentGameMode.IsOperational);
            ...
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.CurrentGameMode != null && m_Process.CurrentGameMode.IsOperational));
        ...
        public void CanBePausedAndRestarted()
        {
            ...
            m_Process.Pause();
            Assert.IsFalse(SimulateProcessUntil(() => m_Process.ServiceHandler.IsOperational, 5));
            m_Process.Restart();
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.ServiceHandler.IsOperational, 5));

            // Pause during the loading of the first game mode
            m_Process.Pause();
            Assert.IsFalse(SimulateProcessUntil(() => m_Process.CurrentGameMode.IsOperational, 5));
            m_Process.Restart();
            Assert.IsTrue(SimulateProcessUntil(() => m_Process.CurrentGameMode.IsOperational, 5));
        ...
        private bool SimulateProcessUntil(Func<bool> condition, int maxFrames = 10)
        {
            int i = 0;
